...
def test_tle_propagate_to_epochs():
    ...
    print(f"  (Should use GPU automatically with threshold=100)")
...
def test_batch_propagator():
    ...
    if gpu_available:
        ...
        print(f"  Propagating with GPU backend...")
        ...
        print(f"  Propagating with CPU backend...")
        ...
        print(f"  ✓ Max CPU vs GPU difference: {max_diff*1000:.1f} m")
        
        if max_diff < 0.1:  # 100m tolerance
            print(f"  ✓ Results match within tolerance")
        else:
            print(f"  ⚠ Warning: Difference exceeds 100m")
    else:
        # CPU only
        propagator.set_backend(PropagationBackend.Cpu)
        print(f"  Propagating with CPU backend (GPU not available)...")
...
def test_constellation_batch():
    ...
    print(f"  Computing batch ephemeris...")
