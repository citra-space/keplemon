...
impl BatchLeastSquares {
    ...
    pub fn get_rms(&self) -> Option<f64> {
        ...
        // Note: For angle observations, range field contains the total position error magnitude
        // (range = sqrt(radial^2 + in_track^2 + cross_track^2)), so we don't need to compute
        // magnitude from RIC components separately to avoid double-counting.
        ...
        for (_, residual) in residuals {
            // For all observation types (angle, TDOA, FDOA), the range field
            // contains the primary error metric we care about
            range_errors.push(residual.get_range());
        }
