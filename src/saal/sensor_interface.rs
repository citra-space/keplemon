...
use super::{astro_func_interface, GetSetString};
use pyo3::prelude::*;
...
impl SAALSensor {
    pub fn new(key: i64) -> Result<SAALSensor, String> {
        let (xa_sen, xs_sen) = SensorInterface::get_arrays(key)?;
        ...
        let lla = SensorInterface::get_lla(key)?;
...
impl SAALSensor {
    #[new]
    pub fn py_new(key: i64) -> PyResult<Self> {
        SAALSensor::new(key).map_err(pyo3::exceptions::PyRuntimeError::new_err)
    }
    #[getter]
    ...
    #[getter]
    pub fn get_minimum_range(&self) -> Option<f64> {
        self.minimum_range
    }

    #[getter]
    pub fn get_maximum_range(&self) -> Option<f64> {
        self.maximum_range
    }

    #[getter]
    pub fn get_range_rate_limit(&self) -> Option<f64> {
        self.range_rate_limit
    }

    #[getter]
    pub fn get_apply_range_limits(&self) -> bool {
        self.apply_range_limits
    }

    #[getter]
    pub fn get_mobile(&self) -> bool {
        self.mobile
    }
    ...
    #[getter]
    pub fn get_astronomical_latitude(&self) -> f64 {
        self.astronomical_latitude
    }

    #[getter]
    pub fn get_astronomical_longitude(&self) -> f64 {
        self.astronomical_longitude
    }

    #[getter]
    pub fn get_azimuth_noise(&self) -> Option<f64> {
        self.azimuth_noise
    }

    #[getter]
    pub fn get_elevation_noise(&self) -> Option<f64> {
    ...
    #[getter]
    pub fn get_azimuth_rate_noise(&self) -> Option<f64> {
        self.azimuth_rate_noise
    }

    #[getter]
    pub fn get_elevation_rate_noise(&self) -> Option<f64> {
        self.elevation_rate_noise
    }

    #[getter]
    pub fn get_angular_noise(&self) -> Option<f64> {
        match (self.azimuth_noise, self.elevation_noise) {
            (Some(az), Some(el)) => Some((az.powi(2) + el.powi(2)).sqrt()),
            _ => None,
        }
    }
...
impl SensorInterface {
    pub fn get(sen_key: i64) -> Result<SAALSensor, String> {
        SAALSensor::new(sen_key)
    }

    pub fn prune_missing_locations() -> Result<(), String> {
        let keys = SensorInterface::get_loaded_keys();

        for key in keys {
            let lla = SensorInterface::get_lla(key).unwrap_or(None);
            if lla.is_none() {
    ...
    pub fn get_all() -> Result<Vec<SAALSensor>, String> {
        let keys = SensorInterface::get_loaded_keys();
        let mut sensors = Vec::new();

        for key in keys {
            let sensor = SAALSensor::new(key)?;
            sensors.push(sensor);
        }
    ...
    pub fn get_lla(sen_key: i64) -> Result<Option<[f64; 3]>, String> {
        ...
        match xa_sen[XA_SEN_GRN_LOCTYPE] as i32 {
            ...
            SENLOC_TYPE_EFG | SENLOC_TYPE_ECR => {
                match astro_func_interface::efg_to_lla(&[
                    xa_sen[XA_SEN_GRN_POS1],
                    xa_sen[XA_SEN_GRN_POS2],
                    xa_sen[XA_SEN_GRN_POS3],
                ]) {
    ...
    pub fn get_loaded_keys() -> Vec<i64> {
        let count = SensorInterface::count_loaded();
        let mut keys = vec![0; count as usize];
        unsafe {
            SensorGetLoaded(MainInterface::IDX_ORDER_ASC as i32, keys.as_mut_ptr());
        }
        keys
    }

    pub fn load_card(card: &str) -> Result<(), String> {
        let mut input_card = GetSetString::from_string(card);
    ...
    pub fn remove_key(sen_key: i64) -> Result<(), String> {
    ...
    pub fn load_file(file_path: &str) -> Result<(), String> {
        let mut input_file = GetSetString::from_string(file_path);
...
impl SensorInterface {
    #[classattr]
    pub const XA_SEN_GEN_SENNUM: usize = XA_SEN_GEN_SENNUM;

    #[classattr]
    pub const XA_SEN_GEN_MINRNG: usize = XA_SEN_GEN_MINRNG;

    #[classattr]
    pub const XA_SEN_GEN_MAXRNG: usize = XA_SEN_GEN_MAXRNG;

    #[classattr]
    pub const XA_SEN_GEN_RRLIM: usize = XA_SEN_GEN_RRLIM;

    #[classattr]
    pub const XA_SEN_GEN_RNGLIMFLG: usize = XA_SEN_GEN_RNGLIMFLG;

    #[classattr]
    pub const XA_SEN_GEN_SMSEN: usize = XA_SEN_GEN_SMSEN;

    #[classattr]
    pub const XA_SEN_GRN_LOCTYPE: usize = XA_SEN_GRN_LOCTYPE;

    #[classattr]
    pub const XA_SEN_GRN_POS1: usize = XA_SEN_GRN_POS1;

    #[classattr]
    pub const XA_SEN_GRN_POS2: usize = XA_SEN_GRN_POS2;

    #[classattr]
    pub const XA_SEN_GRN_POS3: usize = XA_SEN_GRN_POS3;

    #[classattr]
    pub const XA_SEN_GRN_ASTROLAT: usize = XA_SEN_GRN_ASTROLAT;

    #[classattr]
    pub const XA_SEN_GRN_ASTROLON: usize = XA_SEN_GRN_ASTROLON;

    #[classattr]
    pub const XA_SEN_GRN_ECITIME: usize = XA_SEN_GRN_ECITIME;

    #[classattr]
    pub const XA_SEN_GEN_AZSIGMA: usize = XA_SEN_GEN_AZSIGMA;

    #[classattr]
    pub const XA_SEN_GEN_ELSIGMA: usize = XA_SEN_GEN_ELSIGMA;

    #[classattr]
    pub const XA_SEN_GEN_RGSIGMA: usize = XA_SEN_GEN_RGSIGMA;

    #[classattr]
    pub const XA_SEN_GEN_RRSIGMA: usize = XA_SEN_GEN_RRSIGMA;

    #[classattr]
    pub const XA_SEN_GEN_ARSIGMA: usize = XA_SEN_GEN_ARSIGMA;

    #[classattr]
    pub const XA_SEN_GEN_ERSIGMA: usize = XA_SEN_GEN_ERSIGMA;

    #[classattr]
    pub const XA_SEN_GEN_AZBIAS: usize = XA_SEN_GEN_AZBIAS;

    #[classattr]
    pub const XA_SEN_GEN_ELBIAS: usize = XA_SEN_GEN_ELBIAS;

    #[classattr]
    pub const XA_SEN_GEN_RGBIAS: usize = XA_SEN_GEN_RGBIAS;

    #[classattr]
    pub const XA_SEN_GEN_RRBIAS: usize = XA_SEN_GEN_RRBIAS;

    #[classattr]
    pub const XA_SEN_GEN_TIMEBIAS: usize = XA_SEN_GEN_TIMEBIAS;

    #[classattr]
    pub const XA_SEN_SIZE: usize = XA_SEN_SIZE;

    #[staticmethod]
    #[pyo3(name = "get")]
    pub fn py_get(sen_key: i64) -> PyResult<SAALSensor> {
        SensorInterface::get(sen_key).map_err(pyo3::exceptions::PyRuntimeError::new_err)
    }

    #[staticmethod]
    #[pyo3(name = "prune_missing_locations")]
    pub fn py_prune_missing_locations() -> PyResult<()> {
        SensorInterface::prune_missing_locations().map_err(pyo3::exceptions::PyRuntimeError::new_err)
    }

    #[staticmethod]
    #[pyo3(name = "get_all")]
    pub fn py_get_all() -> PyResult<Vec<SAALSensor>> {
        SensorInterface::get_all().map_err(pyo3::exceptions::PyRuntimeError::new_err)
    }

    #[staticmethod]
    #[pyo3(name = "load_file")]
    pub fn py_load_file(file_path: &str) -> PyResult<()> {
        SensorInterface::load_file(file_path).map_err(pyo3::exceptions::PyRuntimeError::new_err)
    }

    #[staticmethod]
    #[pyo3(name = "get_arrays")]
    pub fn py_get_arrays(sen_key: i64) -> PyResult<([f64; XA_SEN_SIZE], String)> {
        SensorInterface::get_arrays(sen_key).map_err(pyo3::exceptions::PyRuntimeError::new_err)
    }

    #[staticmethod]
    #[pyo3(name = "remove_all")]
    pub fn py_remove_all() -> PyResult<()> {
        SensorInterface::remove_all().map_err(pyo3::exceptions::PyRuntimeError::new_err)
    }

    #[staticmethod]
    #[pyo3(name = "get_astronomical_ll")]
    pub fn py_get_astronomical_ll(sen_key: i64) -> PyResult<[f64; 2]> {
        SensorInterface::get_astronomical_ll(sen_key).map_err(pyo3::exceptions::PyRuntimeError::new_err)
    }

    #[staticmethod]
    #[pyo3(name = "get_lla")]
    pub fn py_get_lla(sen_key: i64) -> PyResult<Option<[f64; 3]>> {
        SensorInterface::get_lla(sen_key).map_err(pyo3::exceptions::PyRuntimeError::new_err)
    }

    #[staticmethod]
    #[pyo3(name = "get_loaded_keys")]
    pub fn py_get_loaded_keys() -> PyResult<Vec<i64>> {
        Ok(SensorInterface::get_loaded_keys())
    }

    #[staticmethod]
    #[pyo3(name = "load_card")]
    pub fn py_load_card(card: &str) -> PyResult<()> {
        SensorInterface::load_card(card).map_err(pyo3::exceptions::PyRuntimeError::new_err)
    }

    #[staticmethod]
    #[pyo3(name = "remove_key")]
    pub fn py_remove_key(sen_key: i64) -> PyResult<()> {
        SensorInterface::remove_key(sen_key).map_err(pyo3::exceptions::PyRuntimeError::new_err)
    }

    #[staticmethod]
    #[pyo3(name = "count_loaded")]
    pub fn py_count_loaded() -> PyResult<i32> {
        Ok(SensorInterface::count_loaded())
    }
}
