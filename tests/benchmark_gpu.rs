...
fn benchmark_gpu_propagation() {
    ...
    for (n_sats, n_times) in configs {
        ...
        for _ in 0..n_runs {
            ...
            let _results = gpu_propagator.propagate(&jd_times)
...
fn benchmark_init_vs_propagate() {
    ...
    println!("Configuration: {} satellites × {} times = {} propagations\n", 
             n_sats, n_times, n_sats * n_times);
    println!("Init time:              {:>10.3} ms", init_time.as_secs_f64() * 1000.0);
    println!("First propagate:        {:>10.3} ms (cold)", first_prop_time.as_secs_f64() * 1000.0);
    println!("Subsequent propagate:   {:>10.3} ms (warm, avg of {})", 
             avg_prop_time.as_secs_f64() * 1000.0, n_runs);
