import pytest
from keplemon.bodies import Satellite, Observatory
...
def tle_1():
    ...
    return TLE.from_lines(line_1, line_2)
...
@pytest.fixture()
def sat_1(tle_1):
    return Satellite.from_tle(tle_1)


@pytest.fixture()
def sat_2(tle_2):
    return Satellite.from_tle(tle_2)


@pytest.fixture()
def sat_3(tle_3):
    return Satellite.from_tle(tle_3)


def test_from_tle(sat_1: Satellite, sat_2: Satellite, sat_3: Satellite):

    assert sat_1.norad_id == 25544
    assert sat_2.geodetic_position
    assert sat_3.geodetic_position
    assert sat_2.geodetic_position.latitude == pytest.approx(0.3938497796549098, abs=0.1)
    assert sat_2.geodetic_position.longitude == pytest.approx(55.074384090833696, abs=0.1)
    assert sat_2.geodetic_position.altitude == pytest.approx(35808.08113476326, abs=0.1)


def test_get_close_approach(sat_2: Satellite, sat_3: Satellite):
    ...
    ca = sat_2.get_close_approach(sat_3, start, end, 25.0)
    assert ca
    assert ca.epoch.to_iso() == "2025-04-15T12:32:28.532"
    assert ca.distance == pytest.approx(6.088, abs=0.1)


def test_get_relative_state_at_epoch(sat_2: Satellite, sat_3: Satellite):
    epoch = Epoch.from_iso("2025-04-15T12:32:28.532000Z", TimeSystem.UTC)
    state = sat_2.get_relative_state_at_epoch(sat_3, epoch)
    assert state
    assert state.position.magnitude == pytest.approx(6.088, abs=0.1)
    assert state.position.x == pytest.approx(-3.166, abs=1e-3)
    assert state.position.y == pytest.approx(-5.2, abs=1e-3)
    assert state.position.z == pytest.approx(0.0196, abs=1e-3)
    assert state.velocity.x == pytest.approx(0.001, abs=1e-3)
    assert state.velocity.y == pytest.approx(-0.0003, abs=1e-3)
    assert state.velocity.z == pytest.approx(-0.059, abs=1e-3)


def test_get_body_angles_at_epoch(sat_2: Satellite, sat_3: Satellite):
    epoch = Epoch.from_iso("2025-04-15T12:32:28.532000Z", TimeSystem.UTC)
    angles = sat_2.get_body_angles_at_epoch(sat_3, epoch)
    assert angles
    assert angles.earth_angle == pytest.approx(121.3, abs=0.1)
    assert angles.sun_angle == pytest.approx(121.0, abs=0.1)
    assert angles.moon_angle == pytest.approx(88.0, abs=0.1)


def test_get_observatory_access_report():
    line_1 = "1 25544U 98067A   20200.51605324 +.00000884  00000 0  22898-4 0 0999"
    line_2 = "2 25544  51.6443  93.0000 0001400  84.0000 276.0000 15.4930007023660"
