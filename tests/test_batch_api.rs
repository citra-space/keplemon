...
fn test_batch_propagator_basic() {
    ...
    if BatchPropagator::new().is_gpu_available() {
        ...
        let gpu_results = propagator.propagate_batch(&tles, &epochs)
...
fn test_tle_propagate_batch() {
    ...
    let epochs: Vec<Epoch> = (0..5)
        .map(|i| start + TimeSpan::from_hours(i as f64))
        .collect();
    
    // Test the static batch method
    let results = TLE::propagate_batch(&tles, &epochs)
        .expect("Batch propagation failed");
    ...
    for sat_states in &results {
        for state in sat_states {
            ...
            assert!(pos_mag > 6500.0 && pos_mag < 8000.0,
                "Position magnitude {} km is outside expected range", pos_mag);
        }
    }
}
...
fn test_tle_propagate_to_epochs() {
    ...
    let few_epochs: Vec<Epoch> = (0..10)
        .map(|i| start + TimeSpan::from_minutes(i as f64))
        .collect();
    
    let few_results = tle.propagate_to_epochs(&few_epochs)
        .expect("Few epochs propagation failed");
    
    assert_eq!(few_results.len(), 10);
    
    // Test with many epochs (should use GPU if available)
    let many_epochs: Vec<Epoch> = (0..150)
        .map(|i| start + TimeSpan::from_minutes(i as f64))
        .collect();
    
    let many_results = tle.propagate_to_epochs(&many_epochs)
    ...
    for i in 0..many_results.len()-1 {
        ...
        assert!(pos_diff < 500.0,
            "Position jump too large: {} km", pos_diff);
    }
}
...
fn test_auto_backend_selection() {
    ...
    let _small_epochs: Vec<Epoch> = (0..5)
        .map(|i| start + TimeSpan::from_hours(i as f64))
        .collect();
    ...
    let large_epochs: Vec<Epoch> = (0..150)
        .map(|i| start + TimeSpan::from_minutes(i as f64))
        .collect();
    ...
    let results = propagator.propagate_batch(&tles, &large_epochs)
    ...
    for (sat_id, states) in &results {
        ...
        for (i, state_opt) in states.iter().enumerate() {
            assert!(state_opt.is_some(), 
                "Satellite {} state {} should be Some", sat_id, i);
        }
    }
    
    // Test GPU backend if available
    if Constellation::is_gpu_available() {
        let gpu_results = constellation.get_batch_ephemeris(
            start, end, step, 
...
fn test_gpu_availability_reporting() {
    ...
    assert_eq!(is_available, is_constellation_available,
