...
use keplemon::gpu::{
    CudaGeoNumericalPropagator, CudaTlePropagator, GeoStateGpu, TleDataGpu,
    PropagatorType, DEFAULT_CR, DEFAULT_AREA_MASS,
};
...
fn test_geo_propagation_multi_satellite() {
    ...
    let states: Vec<GeoStateGpu> = (0..10).map(|i| {
    ...
    for sat in 0..10 {
        for t in 0..24 {
            ...
            assert!(r > 35000.0 && r < 50000.0, "Invalid radius {} for sat {} at t={}", r, sat, t);
...
fn test_geo_eci_direct_propagation() {
    ...
    let results = propagator.propagate_eci_direct(&states, &times)
        .expect("Failed to propagate");
...
fn test_geo_custom_srp_parameters() {
    ...
    propagator.init_satellites_with_srp(&positions, &velocities, &epochs, &cr_values, &area_mass)
        .expect("Failed to init");

    let params = propagator.get_params_debug().expect("Failed to get params");
    assert_eq!(params.len(), 1);
    assert!((params[0].cr - 1.5).abs() < 1e-10, "CR not set correctly: {}", params[0].cr);
    assert!((params[0].area_mass - 0.03).abs() < 1e-10, "Area/mass not set correctly: {}", params[0].area_mass);
}
...
fn benchmark_geo_vs_sdp4_performance() {
    ...
    let tle_data: Vec<TleDataGpu> = (0..n_sats).map(|i| {
        TleDataGpu {
            epoch_jd: 2459945.5,
            inclination: 0.05,
            raan: (i as f64) * 3.6,
    ...
    let start = Instant::now();
    let _results = sgp4_prop.propagate_soa_arrays(&times).expect("Failed to propagate SGP4");
    ...
    println!("Speedup:        {:.1}x", sdp4_time.as_secs_f64() / geo_time.as_secs_f64());
}
...
fn test_geo_vs_sdp4_accuracy() {
    ...
    for d in 0..8 {
        ...
        assert!(geo_r > 30000.0 && geo_r < 55000.0, "GEO radius {} out of range at day {}", geo_r, d);
        assert!(sdp4_r > 35000.0 && sdp4_r < 50000.0, "SDP4 radius {} out of range at day {}", sdp4_r, d);
    }
...
fn test_shadow_model_accuracy() {
    ...
    for t in 0..n_times {
        ...
        // Verify no errors
        assert_eq!(results.error_code[t], 0, "Error at timestep {}: {}", t, results.error_code[t]);
    }
    ...
    println!("Radius variation {} km is within expected bounds for 24h propagation.", r_variation);
    ...
    for hour in 0..=6 {
        ...
        if t < n_times {
            let r = radii[t];
            println!("{:4} | {:>11.3} | {:>11.3} | {:>11.3} | {:>10.3}",
                     hour * 4, results.x[t], results.y[t], results.z[t], r);
        }
    }
